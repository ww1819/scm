...
CREATE TABLE IF NOT EXISTS `scm_order` (
  ...
  `supplier_id` bigint(20) DEFAULT NULL COMMENT '订单供应商ID（平台 scm_supplier.supplier_id）',
  `supplier_code` varchar(64) DEFAULT '' COMMENT '平台供应商编码（scm_supplier.supplier_code，与 spd_snapshot_supplier_code 同源）',
  ...
  `warehouse_code` varchar(64) DEFAULT '' COMMENT '订单仓库编码（SPD fd_warehouse.code 快照）',
  `order_dept_id` bigint(20) DEFAULT NULL COMMENT '订单科室ID',
  `order_dept_name` varchar(200) DEFAULT '' COMMENT '订单科室名称',
  `order_dept_code` varchar(64) DEFAULT '' COMMENT '订单科室编码（SPD fd_department.code 快照）',
  `tenant_id` varchar(64) DEFAULT NULL COMMENT '租户ID',
  `order_date` date DEFAULT NULL COMMENT '订单日期',
  `order_amount` decimal(18,6) DEFAULT 0 COMMENT '订单金额',
  `order_status` char(1) DEFAULT '0' COMMENT '订单状态（0待处理 1已确认 2配送中 3已完成 4已取消）',
  `receive_by` varchar(64) DEFAULT NULL COMMENT '接收人登录名',
  `receive_by_name_snapshot` varchar(64) DEFAULT NULL COMMENT '接收人姓名快照（操作时落库，避免用户改名后不可追溯）',
...
CREATE TABLE IF NOT EXISTS `scm_order_detail` (
  ...
  `order_no` varchar(50) DEFAULT '' COMMENT '订单编号（冗余自 scm_order，便于按单号查明细）',
  `hospital_id` bigint(20) DEFAULT NULL COMMENT '医院ID（冗余自 scm_order，便于行级筛选）',
  `hospital_code` varchar(64) DEFAULT '' COMMENT '平台医院编码（冗余自 scm_order.spd_snapshot_hospital_code）',
  `supplier_id` bigint(20) DEFAULT NULL COMMENT '平台供应商ID（冗余自 scm_order.supplier_id）',
  `supplier_code` varchar(64) DEFAULT '' COMMENT '平台供应商编码（冗余自 scm_order）',
  `material_id` bigint(20) NOT NULL COMMENT '物资ID',
  ...
  KEY `idx_order_no` (`order_no`),
  KEY `idx_od_hospital_id` (`hospital_id`),
  KEY `idx_od_supplier_id` (`supplier_id`),
  KEY `idx_od_supplier_code` (`supplier_code`),
  KEY `idx_material_id` (`material_id`),
...
CREATE TABLE IF NOT EXISTS `scm_delivery` (
  ...
  `create_by_name_snapshot` varchar(64) DEFAULT NULL COMMENT '制单人姓名快照（操作时落库，避免用户改名后不可追溯）',
  ...
  `src_order_supplier_id` varchar(128) DEFAULT NULL COMMENT '订单供应商ID(字符串快照)',
  `src_order_supplier_name` varchar(256) DEFAULT NULL COMMENT '订单供应商名称',
  `src_order_warehouse_id` varchar(128) DEFAULT NULL COMMENT '订单仓库ID(字符串快照)',
  `src_order_warehouse_name` varchar(256) DEFAULT NULL COMMENT '订单仓库名称',
  `src_order_dept_id` varchar(128) DEFAULT NULL COMMENT '订单科室ID(字符串快照)',
  `src_order_dept_name` varchar(256) DEFAULT NULL COMMENT '订单科室名称',
  `zs_customer_id` varchar(128) DEFAULT NULL COMMENT '中设客户ID',
  `audit_status` char(1) DEFAULT '0' COMMENT '审核状态（0待审核 1已审核 2已拒绝）',
  `audit_by` varchar(64) DEFAULT NULL COMMENT '审核人',
  `audit_by_name_snapshot` varchar(64) DEFAULT NULL COMMENT '审核人姓名快照（操作时落库，避免用户改名后不可追溯）',
  `audit_time` datetime DEFAULT NULL COMMENT '审核时间',
  `audit_remark` varchar(500) DEFAULT NULL COMMENT '审核备注',
  `zs_jsfs` varchar(32) DEFAULT NULL COMMENT '中设订单结算方式jsfs快照：3高值0低值',
  `spd_tenant_id` varchar(64) DEFAULT NULL COMMENT 'SPD租户ID（同 sb_customer.customer_id）',
  `spd_ref_no` varchar(128) DEFAULT NULL COMMENT 'SPD侧引用/业务流水号（审计）',
  `spd_supplier_id` varchar(64) DEFAULT NULL COMMENT 'SPD采购订单 purchase_order.supplier_id 快照（与 scm_order.spd_supplier_id 一致）',
  `ref_order_source` varchar(16) DEFAULT NULL COMMENT '关联订单来源：SCM第一方 zs第三方',
  `unaudit_by_name_snapshot` varchar(64) DEFAULT NULL COMMENT '反审核人姓名快照（最近一次反审核操作时落库）',
  PRIMARY KEY (`delivery_id`),
  UNIQUE KEY `uk_scm_delivery_tenant_no` (`spd_tenant_id`, `delivery_no`),
...
CREATE TABLE IF NOT EXISTS `scm_delivery_detail` (
  ...
  `order_id` varchar(64) DEFAULT NULL COMMENT '来源订单主键 scm_order.order_id（字符串外键，十进制）',
  `order_no` varchar(50) DEFAULT '' COMMENT '来源订单号（冗余自 scm_order）',
  `combined_id` varchar(36) DEFAULT NULL COMMENT '来源合单ID（UUID7）',
  `combined_no` varchar(50) DEFAULT NULL COMMENT '来源合单号',
  ...
  `model` varchar(200) DEFAULT '' COMMENT '型号',
  `unit` varchar(20) DEFAULT '' COMMENT '单位',
  `remaining_quantity` decimal(18,2) DEFAULT '0.00' COMMENT '剩余配送数量',
  `delivery_quantity` decimal(18,2) DEFAULT '0.00' COMMENT '配送数量',
  `price` decimal(18,6) DEFAULT '0.000000' COMMENT '单价',
  ...
  `aux_barcode` varchar(128) DEFAULT NULL COMMENT '辅条码',
  `pack_coefficient` decimal(18,6) DEFAULT NULL COMMENT '打包系数',
  ...
  KEY `idx_scm_dd_order_id` (`order_id`),
  KEY `idx_combined_id` (`combined_id`),
...
CREATE TABLE IF NOT EXISTS zs_tp_order (
  ...
  confirm_status  CHAR(1)      NOT NULL DEFAULT '0' COMMENT '第三方订单确认：0未确认 1已确认',
  confirm_by        VARCHAR(64)  NULL COMMENT '确认人登录名',
  confirm_by_name_snapshot VARCHAR(64) NULL COMMENT '确认人姓名快照（操作时落库，避免用户改名后不可追溯）',
  confirm_time      DATETIME     NULL COMMENT '确认时间',
  void_status       CHAR(1)      NOT NULL DEFAULT '0' COMMENT '作废：0正常 1已作废',
  void_by           VARCHAR(64)  NULL COMMENT '作废人登录名',
  void_by_name_snapshot VARCHAR(64) NULL COMMENT '作废人姓名快照（操作时落库，避免用户改名后不可追溯）',
