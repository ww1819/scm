/**
 * 产品证件 COS 文件上传（bootstrap-fileinput）
 */
(function (window, $) {
    if (!$) {
        return;
    ...
    function buildPreviewUrl(fileUrl, fileId) {
        ...
        if (fileId) {
            return (ctx.endsWith('/') ? ctx : ctx + '/') + 'certificate/product/downloadFile/' + fileId;
        }
        return '';
    }
    ...
    window.initProductCertCosUpload = function (options) {
        options = $.extend({
        ...
        if (options.initialFiles && options.initialFiles.length > 0) {
            for (var i = 0; i < options.initialFiles.length; i++) {
                ...
                initialPreviewConfig.push({
                    ...
                    key: fileId || fileUrl,
                    downloadUrl: fileId ? (ctx + 'certificate/product/downloadFile/' + fileId) : (fileUrl ? (window.normalizeCosFileUrl ? normalizeCosFileUrl(fileUrl) : fileUrl) : ''),
        ...
        var fileInputOptions = {
            uploadUrl: options.uploadUrl || (ctx + 'certificate/product/uploadFile'),
    ...
    window.readProductCertInitialFilesFromDom = function (elementId) {
